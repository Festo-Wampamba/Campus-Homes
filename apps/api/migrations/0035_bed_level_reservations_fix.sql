-- 0034_bed_level_reservations never successfully applied anywhere (confirmed
-- against both staging and production before this migration was written) —
-- its backfill assigns every reservation on a unit to that unit's *first*
-- bed unconditionally, then tries to make bed_id unique among live
-- (reserved/booked/occupied) reservations. The old schema had no "tenant
-- moved out" step: a `fulfilled` reservation stays `fulfilled` forever even
-- after the unit turns over to a new occupant next term, so real units have
-- more historical `fulfilled` rows than physical beds — the unique index
-- creation fails the instant that happens. 0034's file is left untouched
-- per policy (never modify an existing migration); this migration does the
-- actual work 0034 was supposed to do, correctly. Drizzle records 0034 as
-- applied without ever running its SQL (see the accompanying tracking-table
-- insert run alongside this file) so it isn't retried.

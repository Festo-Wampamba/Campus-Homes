...
export default async function VerificationsPage() {
  ...
  return <><PageHeader eyebrow="Trust operations" title="Verification centre" description="Track property inspections and landlord identity checks from submission through approval." />
    <div className="space-y-5"><AdminExportPanel endpoint="/admin/verifications/export" fixedReportType="verifications" /><SectionCard title="Property visits" description="Physical verification assignments and evidence completion, including completed ones"><AdminTable rows={data?.visits ?? []} filename="campushomes-verification-visits.csv" searchPlaceholder="Search property, inspector, catchment…" columns={[
    ...
    ]} /></SectionCard><SectionCard title="Landlord identity review" description="Confirms who a landlord is — queue and completed decisions"><AdminTable rows={data?.landlordKyc ?? []} filename="campushomes-landlord-kyc.csv" searchPlaceholder="Search landlord, email, phone, status…" columns={[
      { key: "name", label: "Landlord" }, { key: "email", label: "Email" }, { key: "phone", label: "Phone" }, { key: "status", label: "Review status", format: "status" }, { key: "submittedAt", label: "Submitted", format: "date" }, { key: "reviewedAt", label: "Reviewed", format: "date" },

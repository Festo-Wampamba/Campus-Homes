...
export function UsersManager({ rows, roles, permissions, properties, canMutate }: { rows: UserRow[]; roles: Role[]; permissions: Permission[]; properties: Property[]; canMutate: boolean }) {
  ...
  async function revokeRole(id: string) {
    ...
    catch (error) { setNotice(error instanceof ApiError && error.status === 401 ? "This sensitive change needs a recent sign-in — sign out, sign back in, then retry." : apiErrorMessage(error, "The role could not be revoked.")); } finally { setPending(false); }
  }
  ...
  async function removeUser(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!window.confirm(`PERMANENTLY delete ${selected.name || "this user"} and everything they own (properties, listings, reservations, roles)? This is irreversible and cannot be undone.`)) return;
    setPending(true); setNotice(null);
    const id = selected.id;
    try { await api(`/admin/users/${id}`, { method: "DELETE", body: JSON.stringify({ reason: String(data.get("reason")) }) }); setList((current) => current.filter((r) => r.id !== id)); setNotice(`${selected.name || "Account"} permanently deleted.`); router.refresh(); setTimeout(() => close(), 700); }
    catch (error) { setNotice(apiErrorMessage(error, "The user could not be deleted.")); } finally { setPending(false); }
  }
  ...
  async function revokeSessions() {
    ...
    setNotice(null);
    try {
      const result = await api<{ revoked: number }>(`/admin/users/${selected.id}/sessions/revoke`, { method: "POST" });
      setNotice(`${result.revoked} active session${result.revoked === 1 ? "" : "s"} revoked.`);
    } catch { setNotice("Active sessions could not be revoked."); } finally { setPending(false); }
  }
  ...
  function rowActions(row: UserRow) {
    ...
    return <div className="flex gap-1">
      ...
      {canMutate && <button aria-label="Delete user" title="Delete user permanently" onClick={() => { setSelected(row); setMode("delete"); setNotice(null); setFormError(null); }} className="grid size-10 place-items-center rounded-lg border border-red-200 text-red-700 transition-colors hover:border-red-300 hover:bg-red-50 hover:text-red-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 dark:border-red-900 dark:text-red-300 dark:hover:bg-red-950 dark:hover:text-red-200"><Trash2 className="size-4" /></button>}
    </div>;
  }
  ...
  return <>
        ...
        {pageItems.map((row) =><div key={row.id} className="rounded-lg border border-slate-200 p-3.5 dark:border-border">
          <div className="mb-2 flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-wrap gap-1"><span className={`rounded px-1.5 py-1 text-[10px] font-semibold ${row.emailVerified ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-slate-100 text-slate-500 dark:bg-muted dark:text-muted-foreground"}`}>Email {row.emailVerified ? "verified" : "pending"}</span><span className={`rounded px-1.5 py-1 text-[10px] font-semibold ${row.phoneVerified ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-slate-100 text-slate-500 dark:bg-muted dark:text-muted-foreground"}`}>Phone {row.phoneVerified ? "verified" : "pending"}</span>{row.authProviders?.includes("google") && <span className="rounded bg-blue-50 px-1.5 py-1 text-[10px] font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-300">Google</span>}</div>
            </div>
            <StatusBadge value={row.status} />
          </div>
    ...
    <AdminModal open={mode === "access"} onClose={close} title={`Access for ${selected?.name ?? "user"}`} description="Role permissions are combined with an explicit scope. Property access is never inferred from a role alone." wide>
      <section className="mx-5 mt-5 rounded-xl border border-slate-200 p-4 dark:border-border"><div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.12em] text-slate-400">Authentication</p><h3 className="mt-1 text-sm font-bold">Identity and session controls</h3></div><button type="button" onClick={revokeSessions} disabled={pending} className={buttonClass("danger")}>Revoke active sessions</button></div><div className="flex flex-wrap gap-2 text-[11px]"><span className={`rounded-full px-2.5 py-1 font-semibold ${detail?.user.emailVerified ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-slate-100 text-slate-500 dark:bg-muted dark:text-muted-foreground"}`}>Email {detail?.user.emailVerified ? "verified" : "pending"}</span><span className={`rounded-full px-2.5 py-1 font-semibold ${detail?.user.phoneVerified ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-slate-100 text-slate-500 dark:bg-muted dark:text-muted-foreground"}`}>Phone {detail?.user.phoneVerified ? "verified" : "pending"}</span>{(detail?.user.authProviders ?? []).map((provider) => <span key={provider} className="rounded-full bg-blue-50 px-2.5 py-1 font-semibold capitalize text-blue-700 dark:bg-blue-950 dark:text-blue-300">{provider === "credential" ? "Email/password" : provider}</span>)}</div></section>

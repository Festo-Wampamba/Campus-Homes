...
import { normalizeUgPhone } from "@campushomes/shared";
...
export function UsersManager({ rows, roles, permissions, properties, canMutate }: { rows: UserRow[]; roles: Role[]; permissions: Permission[]; properties: Property[]; canMutate: boolean }) {
  ...
  function normalizePhoneField(key: "phone" | "emergencyContactPhone") {
    const value = form[key];
    if (!value) return;
    const normalized = normalizeUgPhone(value);
    if (/^\+2567\d{8}$/.test(normalized)) setField(key, normalized);
  }
  ...
  return <>
    <div className="flex flex-col gap-3 border-b border-slate-200 p-3 sm:flex-row dark:border-border">
      <label className="relative min-w-0 flex-1"><span className="sr-only">Search users</span><Search aria-hidden className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input className={`${adminFieldClass} pl-9`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, email, phone, role, campus…" /></label>
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-xs"><thead className="bg-slate-50 text-slate-500 dark:bg-muted"><tr>{["User", "Contact", "Account type", "Staff / access roles", "Campus", "KYC", "Status", "Joined", "Actions"].map((label) => <th key={label} className="px-4 py-3 font-bold uppercase tracking-[.06em]">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100 dark:divide-border">{filtered.map((row) => <tr key={row.id} className="hover:bg-slate-50/80 dark:hover:bg-muted/30"><td className="px-4 py-3.5 font-bold">{row.name || "Unnamed user"}</td><td className="px-4 py-3.5 text-slate-500"><span className="block">{row.email || "—"}</span><span className="block text-[10px]">{row.phone || "—"}</span></td><td className="px-4 py-3.5 capitalize">{row.role.replaceAll("_", " ")}</td><td className="max-w-64 px-4 py-3.5"><div className="flex flex-wrap gap-1">{row.assignments?.length ? row.assignments.map((assignment) => <span key={assignment.id ?? `${assignment.key}-${assignment.scopeId}`} className="rounded bg-slate-100 px-1.5 py-1 text-[10px] font-semibold dark:bg-muted">{assignment.name ?? assignment.key}{assignment.scopeId ? ` · ${properties.find((item) => item.id === assignment.scopeId)?.name ?? assignment.scopeId}` : ""}</span>) : "—"}</div></td><td className="px-4 py-3.5">{row.university ?? "—"}</td><td className="px-4 py-3.5"><StatusBadge value={row.kycStatus ?? "not applicable"} /></td><td className="px-4 py-3.5"><StatusBadge value={row.status} /></td><td className="px-4 py-3.5 text-slate-500">{row.createdAt ? new Date(String(row.createdAt)).toLocaleDateString() : "—"}</td><td className="px-4 py-3.5"><div className="flex gap-1"><button aria-label="Edit user" title="Edit particulars" onClick={() => startDetail(row, "edit")} className="grid size-10 place-items-center rounded-lg border border-slate-200 text-slate-600 transition-colors hover:border-teal-300 hover:bg-teal-50 hover:text-teal-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600 dark:border-border dark:text-slate-300 dark:hover:border-teal-800 dark:hover:bg-teal-950 dark:hover:text-teal-200"><Pencil className="size-4" /></button><button aria-label="Manage access" title="Roles and permissions" onClick={() => startDetail(row, "access")} className="grid size-10 place-items-center rounded-lg border border-slate-200 text-slate-600 transition-colors hover:border-violet-300 hover:bg-violet-50 hover:text-violet-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:border-border dark:text-slate-300 dark:hover:border-violet-800 dark:hover:bg-violet-950 dark:hover:text-violet-200"><KeyRound className="size-4" /></button>{canMutate && <button aria-label="Delete user" title="Soft-delete user" onClick={() => { setSelected(row); setMode("delete"); setNotice(null); setFormError(null); }} className="grid size-10 place-items-center rounded-lg border border-red-200 text-red-700 transition-colors hover:border-red-300 hover:bg-red-50 hover:text-red-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 dark:border-red-900 dark:text-red-300 dark:hover:bg-red-950 dark:hover:text-red-200"><Trash2 className="size-4" /></button>}</div></td></tr>)}</tbody></table>{!filtered.length && <p className="py-14 text-center text-sm text-slate-500">No users match the current search.</p>}</div>
    <div className="border-t border-slate-100 px-4 py-2 text-[11px] text-slate-500 dark:border-border">Showing {filtered.length} of {rows.length} active user records</div>
    ...
    <AdminModal open={mode === "create" || mode === "edit"} onClose={close} title={mode === "create" ? "Add a CampusHomes user" : `Edit ${selected?.name ?? "user"}`} description="Identity and particulars are separate from role assignments and property access." wide>
      <form onSubmit={saveUser} className="space-y-5 p-5">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <AdminField label="Phone" hint="Local and spaced formats are accepted and saved as +256 E.164.">
            <input type="tel" inputMode="tel" autoComplete="tel" className={adminFieldClass} placeholder="+256 771 234 567" value={form.phone} onChange={(event) => setField("phone", event.target.value)} onBlur={() => normalizePhoneField("phone")} />
          </AdminField>
          ...
          {mode === "create" && <AdminField label="Temporary password" hint="Optional; minimum 16 characters and requires email.">
            <input type="password" minLength={16} autoComplete="new-password" className={adminFieldClass} placeholder="At least 16 characters" value={form.temporaryPassword} onChange={(event) => setField("temporaryPassword", event.target.value)} />
          </AdminField>}
          {form.accountType === "student" && <>
            <AdminField label="University">
            ...
            <AdminField label="Year of study">
          ...
          {form.accountType === "landlord" && <AdminField label="Legal / business name">
        ...
        <div className="border-t border-slate-100 pt-5 dark:border-border">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <AdminField label="Emergency phone"><input type="tel" inputMode="tel" className={adminFieldClass} placeholder="+256 772 345 678" value={form.emergencyContactPhone} onChange={(event) => setField("emergencyContactPhone", event.target.value)} onBlur={() => normalizePhoneField("emergencyContactPhone")} /></AdminField>
          </div>

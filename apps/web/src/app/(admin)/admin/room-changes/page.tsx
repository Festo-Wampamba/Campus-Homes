...
export default async function AdminRoomChangesPage() {
  const items = await apiServerStrict<RoomChangeReviewItem[]>("/ops/room-change-requests");
  return <><h1 className="text-2xl">Room change reviews</h1><p className="mt-1 text-sm text-muted-foreground">Approve room types and inventory changes before they become bookable.</p><RoomChangeReviewManager initialItems={items} /></>;
}

...
export function PropertiesManager({ rows, landlords, semesters, permissions }: { rows: PropertyRow[]; landlords: Landlord[]; semesters: Semester[]; permissions: string[] }) {
  ...
  function startCreate() { const firstSemester = semesters.find((semester) => !semester.university || semester.university === emptyForm.catchment); setForm({ ...emptyForm, landlordId: landlords[0]?.id ?? "", semesterId: firstSemester?.id ?? "" }); setAmenities({}); setUnits([emptyUnit()]); setImageUrls(""); setImageFiles([]); setNotice(null); setMode("create"); }
  ...
  return <>
    ...
    <AdminModal open={mode === "create" || mode === "edit"} onClose={close} title={mode === "create" ? "Add a property" : `Update ${selected?.name ?? "property"}`} description="Create the property record, operational utilities, media, and room inventory without changing Ops-controlled verification or publication status." wide>
      <form onSubmit={save} className="space-y-6 p-5"><div className="grid gap-4 md:grid-cols-3"><AdminField label="Landlord / owner"><select required className={adminFieldClass} value={form.landlordId} onChange={(e) => setField("landlordId", e.target.value)}><option value="">Select owner</option>{landlords.map((landlord) => <option key={landlord.id} value={landlord.id}>{landlord.name} · {landlord.email}</option>)}</select></AdminField><AdminField label="Property name"><input required className={adminFieldClass} placeholder="Makerere View Hostel" value={form.name} onChange={(e) => setField("name", e.target.value)} /></AdminField><AdminField label="Property type"><select className={adminFieldClass} value={form.type} onChange={(e) => setField("type", e.target.value)}>{TYPES.map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select></AdminField><div className="md:col-span-2"><AdminField label="Street address"><input required className={adminFieldClass} placeholder="Plot 12, Makerere Hill Road" value={form.streetAddress} onChange={(e) => setField("streetAddress", e.target.value)} /></AdminField></div><AdminField label="University catchment"><select className={adminFieldClass} value={form.catchment} onChange={(e) => setCatchment(e.target.value)}>{CATCHMENTS.map((item) => <option key={item}>{item}</option>)}</select></AdminField><AdminField label="Operational status"><select className={adminFieldClass} value={form.operationalStatus} onChange={(e) => setField("operationalStatus", e.target.value)}>{["open", "temporarily_closed", "under_renovation", "emergency_closure"].map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select></AdminField><AdminField label="Contact phone"><input type="tel" inputMode="tel" className={adminFieldClass} placeholder="+256 771 234 567" value={form.contactPhone} onChange={(e) => setField("contactPhone", e.target.value)} /></AdminField><AdminField label="Contact email"><input type="email" className={adminFieldClass} placeholder="manager@example.com" value={form.contactEmail} onChange={(e) => setField("contactEmail", e.target.value)} /></AdminField></div><AdminField label="Description"><textarea className={adminTextareaClass} placeholder="Describe the location, rooms, access, and notable facilities…" value={form.description} onChange={(e) => setField("description", e.target.value)} /></AdminField>
